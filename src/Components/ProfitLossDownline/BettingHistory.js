...
import { getClientBetHistory, getClientSportbookBetHistory } from '../../Redux/action/Account';
...
const BettingHistory = (props) => {
    ...
    const getBetHistory = (option) => {
        ...
        setLoading(true);
        if (select === 1) {
            dispatch(getClientSportbookBetHistory({ sid: token, clientId: (selectedItem.clientId) ? selectedItem.clientId : selectedItem.agentId, betStatus: betStatus, startDate: start, endDate: end, view: 1 })).then((response) => {
                ...
                {dataExist && <table id="matchTable" className="table-s" style={{ display: 'table' }}>
                    <tbody><tr>
                        ...
                        {bethistory.length > 0 && bethistory.map((item, index) => {
                            ...
                            } else if (item.betType == 'sportbook') {
                                odds = parseFloat(parseFloat(item.rate)).toFixed(2);
                                runner = item?.teamName;
