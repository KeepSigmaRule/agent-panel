...
const [minPageNumberLimit, setminPageNumberLimit] = useState(0);
  useEffect(()=>{
    ...
    dispatch(getAgentAccountStatement({sid:token,agentId:props.selectedAgentId,agentLevel:props.selectedAgentLevel})).then((response)=>{
            ...
            <tbody>
                <tr>
                    <td className="game-name">
                        Banking Logs
                        <span style={{right:'1024px'}} id="title_userId" >{props.selectedAgentId}</span>
                        <Link to="" style={{top:'-1px'}} onClick={()=>props.setshowDownlineLogs(false)} className="pop-close" ><img className="icon-back" src={Transparent}/>Close</Link>
                    </td>
                </tr>
            </tbody>
            </table>
